...
public class DeviceOperateRequest {
    ...
    public static DeviceOperateResponse operate(DeviceOperate operate, String devToken, JLinkClient jClient) {
        ...
        String requestDeviceOperateUrl = String.format("%s/%s/%s", JLinkDomain.OPENAPI_DOMAIN.get(), JLinkDeviceRequestUrl.DEVICE_OPDEV.get(), devToken);
        //send https request
        String res = JLinkHttpUtil.httpsRequest(requestDeviceOperateUrl, JLinkMethodType.POST.get(), JLinkHeaderUtil.map(jClient), new Gson().toJson(operate));
        try {
            res = formatRes(res);
