...
public class DeviceAbilityRequest {
     ...
     * Get Device Ability
     */
    public DeviceAbilityResponse.DevAbility deviceAbility(JLinkDeviceAbilityEnum abilityEnum, String devToken, JLinkClient mJLinkClient) {
        ...
        String requestDeviceAbilityUrl = String.format("%s/%s/%s", JLinkDomain.OPENAPI_DOMAIN.get(), JLinkDeviceRequestUrl.DEVICE_GETABILITY.get(), devToken);
        Map<String, String> param = new HashMap<>(1);
        param.put("Name", abilityEnum.getAbility());
        //send https request
        String res = JLinkHttpUtil.httpsRequest(requestDeviceAbilityUrl, JLinkMethodType.POST.get(), JLinkHeaderUtil.map(mJLinkClient), new Gson().toJson(param));
        try {
